...
from host_application.host_device_serial import HostSerial                      # lower layer serial driver
from host_application.state_proto_defs import Communication, NodeState, Debug   # protobuf message definitions
from host_application.state_proto_node_default import NodeStateDefaults         # protobuf message defaults
...
class HostDeviceStateSerdes:
    ...
    def __init__(
        ...
        *,
        ...
        self.port = HostSerial(device_serial_regex=device_serial_regex, logger=self.log)
        ...
        # generic publish cache (topic -> last value) for change-only publishing
        # useful for port state publishes
    ...
    def _configure_sub_port_state(self) -> None:
        ...
        #and register this callback to handle connection request messages
        def _on_request_connect(payload: Any = None, **kwargs) -> None:
            if "payload" not in kwargs and payload is None:
                self.log.warning("request_connect callback invoked without 'payload' kwarg")
                return
    ...
    def _pub_change(self, topic: str, value: Any) -> None:
        """
        Publish 'value' to 'topic' only if the value differs from the cached last value.
        Update the cache on publish.
        """
        if topic in self._pub_cache and self._pub_cache[topic] == value:
            return
        pub.sendMessage(topic, payload=value)
        self._pub_cache[topic] = value
    ...
    def _configure_sub_node_state(self) -> None:
        ...
        def _on_node_command(payload: Any = None, **kwargs) -> None:
            if "payload" not in kwargs and payload is None:
                self.log.warning("node command callback invoked without 'payload' kwarg")
                return
        ...
        pub.subscribe(_on_node_command, f"{self.root}.command") 
