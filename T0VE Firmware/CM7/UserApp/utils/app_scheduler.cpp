...
void Scheduler::schedule_oneshot_ms(Callback_Function<> _cb, uint32_t _interval_ms) {
    schedule_nshot_ms(_cb, _interval_ms, 1);
}

void Scheduler::schedule_nshot_ms(Callback_Function<> _cb, uint32_t _interval_ms, uint32_t count) {
...
void Scheduler::schedule_interval_ms(Callback_Function<> _cb, uint32_t _interval_ms) {
    //to keep things simple, we'll start by descheduling the task
    deschedule();
...
void Scheduler::check_run_task() {
    ...
    uint32_t time_of_check = Tick::get_ms();
