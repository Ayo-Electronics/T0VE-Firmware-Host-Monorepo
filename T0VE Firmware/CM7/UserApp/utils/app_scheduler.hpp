...
public:
    ...
    static const uint32_t INTERVAL_EVERY_ITERATION = 0; //use this if we wanna call a function every single time the scheduler loop is called
    static const uint32_t ONESHOT_NEXT_ITERATION = 0; //use this if we'd like to defer a function call to the iteration of scheduler loop
    ...
    void schedule_oneshot_ms(Callback_Function<> _cb, uint32_t _interval_ms);
    void schedule_nshot_ms(Callback_Function<> _cb, uint32_t _interval_ms, uint32_t count);
    void schedule_interval_ms(Callback_Function<> _cb, uint32_t _interval_ms);
...
private:
    ...
    Callback_Function<> cb = Callback_Function<>(); //function we run, default initialized to empty
